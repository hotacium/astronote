...
impl Config {
    pub fn try_new() -> Result<Self, Box<dyn std::error::Error + Send + Sync + 'static>> {
        ...
        let mut config: Self = builder.extract()?;
...
pub fn find_config_root_absolute(builder: &Figment) -> Result<String, Box<dyn std::error::Error + Send + Sync + 'static>> {
    let path_string = find_config_root(builder)?;
    let path = Path::new(&path_string).canonicalize()?;
    Ok(path.to_string_lossy().to_string())
}

fn find_config_root(builder: &Figment) -> Result<String, Box<dyn std::error::Error + Send + Sync + 'static>> {
    
    // 1. If `root` is set in config file, then root is where the config file resides
    let root_metadata = builder.find_metadata("root").ok_or("Unable to get metadata of root")?;
    if root_metadata.name.starts_with("TOML") {
        let path_string = builder.find_value("root")?.into_string().ok_or("`root` in config file is not string")?;
    ...
    if let Some(config_path) = config_path {
        let config_root = config_path.strip_suffix(CONFIG_FILE_NAME).unwrap().to_string();
